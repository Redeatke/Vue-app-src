...
const displayMembers = computed(() => {
  ...
  const raw = chatInfo.value?.members ?? []

  // Normalise each member to { key, label }
  const list = raw.map((m) => ({
    key: m._id ?? m.userId ?? m.username ?? String(m),
...
const isOwner = computed(() => {
  ...
  const ownerId = (typeof owner === 'string' ? owner : owner._id ?? owner.id ?? '').toString()
...
async function leaveChat() {
  ...
  let result
  if (isOwner.value) {
    // Owners can't leave — they must delete the chat
    result = await chatStore.deleteChat(chatId.value)
  } else {
    result = await chatStore.leaveChat(chatId.value)
  }
  if (result.success) {
...
function resolveSender(raw) {
  ...
  const members = chatInfo.value?.members ?? []
  const found = members.find((m) => m._id === raw || m.id === raw || m.userId === raw || m.username === raw)
...
<template>
  <div class="group-chat-view">
    ...
    <div class="chat-header">
      ...
      <button class="leave-btn" @click="showLeaveModal = true" :disabled="isLeaving">
        {{ isLeaving ? '…' : (isOwner ? 'Delete Chat' : 'Leave Chat') }}
      </button>
    </div>
  ...
  <ConfirmModal
    ...
    :message="isOwner ? 'You are the owner. This will permanently DELETE the chat for everyone. Continue?' : 'Are you sure you want to leave this chat?'"
    :confirmLabel="isOwner ? 'Delete Chat' : 'Leave Chat'"
    @confirmed="leaveChat"
  />
</template>
