...
export const useUserStore = defineStore('user', () => {
    ...
    async function createAccount(userData) {
        try {
            // PRE-VALIDATION WORKAROUND:
            // Since the backend lacks unique database constraints for usernames,
            // we must check for duplicates ourselves. Because /users is protected,
            // we secretly log into a dedicated validator account to inspect the database.
            let loginRes = await fetch(API_HOST + '/user/login', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ username: '_validator_', password: 'ValidatorPassword1!' })
            })

            // Auto-create validator if missing
            if (!loginRes.ok) {
                await fetch(API_HOST + '/user', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ username: '_validator_', password: 'ValidatorPassword1!', firstName: 'V', lastName: 'V', email: 'v@v.com' })
                })
                loginRes = await fetch(API_HOST + '/user/login', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ username: '_validator_', password: 'ValidatorPassword1!' })
                })
            ...
            if (loginRes.ok) {
                ...
                const searchRes = await fetch(`${API_HOST}/users?search=${encodeURIComponent(userData.username)}`, {
                    headers: { 'Authorization': `Bearer ${authToken}` }
                })
                if (searchRes.ok) {
                    const searchData = await searchRes.json()
                    const duplicate = (searchData.users || []).some(
                        u => u.username.toLowerCase() === userData.username.toLowerCase()
                    )
                    if (duplicate) {
                        return { success: false, errors: [`Username "${userData.username}" is already taken.`] }
                    }
                }
            }

            // Database is clear, proceed with creating the requested account
    ...
    async function fetchAllUsers() {
        try {
            ...
            while (skip < total) {
                const response = await fetch(`${API_HOST}/users?skip=${skip}`, { headers: authHeaders() })
                ...
                for (const u of fetchedUsers) {
                    userCache.value[u._id] = u.username
                }
                
                if (fetchedUsers.length === 0) break
                skip += fetchedUsers.length
            }
        } catch {}
    }
    ...
    async function removeFriend(username) {
        try {
            // Resolve username → _id (friends array may only hold usernames)
            const searchRes = await fetch(`${API_HOST}/users?search=${encodeURIComponent(username)}`, { headers: authHeaders() })
            const searchData = await searchRes.json()
            const target = (searchData?.users || []).find(u => u.username === username)
            ...
            const response = await fetch(`${API_HOST}/friend/${target._id}`, { method: 'DELETE', headers: authHeaders() })
            if (response.ok) await getUser()
            return { success: response.ok, message: response.ok ? 'Friend removed.' : 'Failed to remove friend.' }
        } catch {
            return { success: false, message: 'Network error.' }
        }
    }
