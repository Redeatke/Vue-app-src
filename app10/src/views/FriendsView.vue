...
// Remove friend confirm modal state
...
async function addFriend() {
  ...
  if (target === userStore.currentUser?.username) {
    feedback.value = "You can't send a friend request to yourself."
    return
  }

  const alreadyFriend = (userStore.currentUser?.friends || []).some(
    f => (f.username || f) === target
  )
  if (alreadyFriend) {
    feedback.value = `${target} is already your friend.`
    return
  }
...
function askRemoveFriend(friend) {
...
async function confirmRemoveFriend() {
  ...
  const username = friendToRemove.value.username || friendToRemove.value
  const result = await userStore.removeFriend(username)
  feedback.value = result.message
...
<template>
  <div class="friends-view">
    ...
    <div class="section">
      ...
      <ul v-if="userStore.friendRequests.length > 0">
        <li v-for="req in userStore.friendRequests" :key="req._id">
    ...
    <div class="section">
      ...
      <ul v-if="userStore.currentUser?.friends?.length > 0">
        ...
        >
          ...
          <button class="remove-btn" @click="askRemoveFriend(friend)">Remove</button>
        </li>
      </ul>
  ...
  <ConfirmModal
    ...
    :message="`Remove ${friendToRemove?.username || friendToRemove} from your friends?`"
    confirmLabel="Remove"
