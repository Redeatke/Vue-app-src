...
function addFriend() {
  ...
  const result = userStore.sendFriendRequest(target)
...
function acceptRequest(fromUsername) {
  const result = userStore.acceptFriendRequest(fromUsername)
  feedback.value = result.message
}
</script>
...
<template>
  <div class="friends-view">
    ...
    <div class="section">
      ...
      <ul v-if="userStore.currentUser?.pending.length > 0">
        <li
          v-for="req in userStore.currentUser.pending"
          :key="req"
        >
          {{ req }}
